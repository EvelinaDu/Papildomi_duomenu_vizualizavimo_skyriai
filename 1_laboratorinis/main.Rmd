...
```{r}
data_outlier_info <- data_outlier_info %>%
  mutate(is_outlier = as.integer(outlier_dim > 0) )
```
...
data_outlier_info %>%
  ...
  summarise(sum = sum(is_outlier),
            n = n(),
            value = round(sum/n, 2)) %>% 
  ggplot(aes(x = purpose, y = credit.policy, size = value, color = sum)) +
  geom_point() +
  geom_text(aes(label = value), 
            size = 6,
            vjust = -2,
            show.legend = FALSE) +
  labs(x = "", y = "Atitinka kredito politika", title = "Ekstremalios išskirtys") +
  scale_size(range = c(2, 20)) +
  theme_bw() +
  theme(legend.position = "none",
    axis.text.x = element_text(angle = 45, vjust = 0.6, size = 11),
